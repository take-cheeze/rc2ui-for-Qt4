...
{
    wi(); *out << "<property name=\"" << name << "\">" << endl; indent();    wi(); *out << "<cstring>" << value << "</cstring>" << endl; undent();
    wi(); *out << "</property>" << endl;
}

void RC2UI::writeString( const QString& name, const QString& value )
{
    wi(); *out << "<property name=\"" << name << "\">" << endl; indent();    wi(); *out << "<string>" << value << "</string>" << endl; undent();
    wi(); *out << "</property>" << endl;
}

void RC2UI::writeRect( const QString& name, int x, int y, int w, int h )
{
    wi(); *out << "<property name=\"" << name << "\">" << endl; indent();    wi(); *out << "<rect>" << endl; indent();
...
{
    wi(); *out << "<property name=\"" << name << "\">" << endl; indent();    wi(); *out << "<bool>" << (value ? "true" : "false") << "</bool>" << endl; undent();
    wi(); *out << "</property>" << endl;
}

void RC2UI::writeNumber( const QString& name, int value )
{
    wi(); *out << "<property name=\"" << name << "\">" << endl; indent();    wi(); *out << "<number>" << value << "</number>" << endl; undent();
    wi(); *out << "</property>" << endl;
}

void RC2UI::writeEnum( const QString& name, const QString& value )
{
    wi(); *out << "<property name=\"" << name << "\">" << endl; indent();    wi(); *out << "<enum>" << value << "</enum>" << endl; undent();
    wi(); *out << "</property>" << endl;
}

void RC2UI::writeSet( const QString& name, const QString& value )
{
    wi(); *out << "<property name=\"" << name << "\">" << endl; indent();    wi(); *out << "<set>" << value << "</set>" << endl; undent();
    wi(); *out << "</property>" << endl;
}
...
bool RC2UI::makeDialog()
{
	...
	for ( endDesc = 0; endDesc < line.length() ; endDesc++ ) {
	    char c = line.at(endDesc).toAscii();
	    if ( space && (c >= '0') && (c <= '9') )
		break;
	    space = c==' ';
	}
	...
	className = parseNext( desc, ' ' ).toAscii();

 	count = sscanf( line.toStdString().c_str(), "%d, %d, %d, %d", &x, &y, &w, &h );
	...
	do {
	    ...
	    count = sscanf( line.toStdString().c_str(), "%s", property );
	...
	*out << "</UI>" << endl;
	...
	if ( writeToFile )
	    puts( (QDir::currentPath() + "/" + fileOut.fileName() + "\n").toStdString().c_str() );
...
bool RC2UI::makeStringTable()
{
	...
	char discard[12];
	sscanf( line.toStdString().c_str(), "%s %s", stringtable, discard );
